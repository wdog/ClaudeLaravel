...
# Create vite.config.js for both dev and prod
...
export default defineConfig(({ mode }) => {
    ...
    // Check if SSL certificates exist (only in Docker container during dev)
    const sslKeyPath = '/etc/nginx/ssl/nginx.key';
    const sslCertPath = '/etc/nginx/ssl/nginx.crt';
